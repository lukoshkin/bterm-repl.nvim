...
function M.setup(conf)
  ...
  conf.valid_buffers = vim.tbl_keys(conf.cell_delimiters)

  conf.pats = {}
  conf.pats = vim.tbl_map(function(delim_list)
    return table.concat(
      vim.tbl_map(function(delim)
        return "^\\s*" .. delim .. ".*"
      end, delim_list),
      "\\|"
    )
  end, conf.cell_delimiters)
  ...
  vim.keymap.set("n", conf.keys.toggle_separator_backward, function()
    repl.toggle_separator(1, 2)
  end)
